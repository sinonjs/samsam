...
describe("match", function () {
    ...
    if (typeof Set !== "undefined") {
        it("Should pass if sets with same content", function () {
            var checkMatch = samsam.match(new Set([1, 2, 3]), new Set([2, 3, 1])); // eslint-disable-line
            assert.equals(checkMatch, true);
        });

        it("Should pass if subsets", function () {
            var checkMatch = samsam.match(new Set([1, 2, 3]), new Set([3, 1])); // eslint-disable-line
            assert.equals(checkMatch, true);
        });

        it("Should pass if subset complex types", function () {
            var checkMatch = samsam.match(new Set([1, {id: 42}, 3]), new Set([{id: 42}])); // eslint-disable-line
            assert.equals(checkMatch, true);
        });

        it("Should fail if sets with dissimilar content", function () {
            var checkMatch = samsam.match(new Set([1, 2, 3]), new Set([2, 5, 1])); // eslint-disable-line
            refute.equals(checkMatch, true);
        });

        it("Should fail if sets with different complex member", function () {
            var checkMatch = samsam.match(new Set([{id: 42}]), new Set([{id: 13}])); // eslint-disable-line
            refute.equals(checkMatch, true);
        });
